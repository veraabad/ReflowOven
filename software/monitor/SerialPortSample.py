...
#command2 = "debug command dsp_device emi 136000000 450000000 60000 100 25000"
...
#minutes = 8
#minutes = 6
print "Wait",minutes,"mins, then go get radio"
loop = True
while loop:
    time.sleep(1)
    timeRemaining = int(time.time() - timeStart) - minutes * 60
    minutesRem = (int(timeRemaining / 60) * -1) - 1
    if minutesRem < 0:
        minutesRem = 0
    seconds = 60 - int(timeRemaining % 60)
    if seconds == 60:
        seconds = 0
    print minutesRem, seconds
    if timeRemaining == 0:
        loop = False
...
countNil = 0
while 1:
    timeLeft = int(time.time() - timeOut) - seconds
    ...
    for line in f:
        ...
        if not contains_digits(line):
            ...
            if holder.isdigit():
                ...
                lineHolder = line.rstrip('\r\n')
                graph.write(lineHolder + '\n')
            else:
                print "No float here"
                continue
                time.sleep(10)
